...
def crear_tamano(tamano: TamanoCreate):
    conn = get_connection()
    try:
        cursor = conn.execute("INSERT INTO tamanos (nombre_tamano) VALUES (?)", (tamano.nombre_tamano,))
...
def obtener_tamano(id: int):
    conn = get_connection()
    t = conn.execute("SELECT * FROM tamanos WHERE id = ?", (id,)).fetchone()
    conn.close()
...
def actualizar_tamano(id: int, tamano: TamanoCreate):
    conn = get_connection()
    if not conn.execute("SELECT id FROM tamanos WHERE id = ?", (id,)).fetchone():
        conn.close(); error_404("Tamano", id)
    conn.execute("UPDATE tamanos SET nombre_tamano = ? WHERE id = ?", (tamano.nombre_tamano, id))
...
def eliminar_tamano(id: int):
    ...
    if not conn.execute("SELECT id FROM tamanos WHERE id = ?", (id,)).fetchone():
        conn.close(); error_404("Tamano", id)
    conn.execute("DELETE FROM tamanos WHERE id = ?", (id,))
    conn.commit(); conn.close()
