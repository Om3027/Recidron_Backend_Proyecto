...
def crear_geo(geo: GeoCreate):
    ...
    conn = get_connection()
    if not conn.execute("SELECT id FROM reportes WHERE id = ?", (geo.reporte_id,)).fetchone():
        conn.close(); raise HTTPException(status_code=404, detail="El reporte no existe")
    try:
        cursor = conn.execute(
            "INSERT INTO geolocalizaciones (latitud, longitud, altitud, precision, reporte_id) VALUES (?,?,?,?,?)",
...
def obtener_geo(id: int):
    conn = get_connection()
    g = conn.execute("SELECT * FROM geolocalizaciones WHERE id = ?", (id,)).fetchone()
    conn.close()
...
def actualizar_geo(id: int, datos: GeoUpdate):
    conn = get_connection()
    if not conn.execute("SELECT id FROM geolocalizaciones WHERE id = ?", (id,)).fetchone():
        conn.close(); error_404("Geolocalizacion", id)
    campos = {k: v for k, v in datos.dict().items() if v is not None}
    if campos:
        set_clause = ", ".join([f"{k} = ?" for k in campos])
        conn.execute(f"UPDATE geolocalizaciones SET {set_clause} WHERE id = ?", list(campos.values()) + [id])
        conn.commit()
    conn.close()
...
def eliminar_geo(id: int):
    ...
    if not conn.execute("SELECT id FROM geolocalizaciones WHERE id = ?", (id,)).fetchone():
        conn.close(); error_404("Geolocalizacion", id)
    conn.execute("DELETE FROM geolocalizaciones WHERE id = ?", (id,))
